...
class Game:
    def __init__(self):
        ...
        self.graphics = Graphics(self, SQUARE_SIZE)
    ...
    def run(self):
        while True:
            ...
            if (self.currentState == self.gameState.PICKUP):
                if (self.board.board[self.coords[0]][self.coords[1]]): 
                    self.piece = self.board.pickupPiece(self.coords)
                self.currentState = self.gameState.HOLDPIECE

            elif (self.currentState == self.gameState.HOLDPIECE):
                if (self.piece):
                    pos = pygame.mouse.get_pos()
                    self.graphics.printBoard()
                ...
                if (self.piece):
                    pos = pygame.mouse.get_pos()
                    self.coords = self.graphics.calculateSquare(pos, self.board, SQUARE_SIZE)
                    if self.board.inBounds(self.coords[0], self.coords[1]):
                        if self.board.isPromotion(self.piece, self.coords):
                            if self.board.availableMoves(self.board.heldPiece[0], self.board.heldPiece[1])[self.coords[0]][self.coords[1]] and not self.board.moveInCheck(self.board.heldPiece, coords):
                                self.graphics.printBoard()
                                self.currentState = self.gameState.PROMOTE
                                self.promoteButtons = self.graphics.generateButtons(self.manager, self.board, self.coords, SQUARE_SIZE)
                                self.promoteSquare = self.coords
                        else:
                            self.board.finalizeMove(self.board.heldPiece, self.coords, None, self)
    ...
    def manageEvents(self, events):
        for event in events:
            ...
            if event.type == pygame.MOUSEBUTTONDOWN: 
                pos = pygame.mouse.get_pos()
                self.coords = self.graphics.calculateSquare(pos, self.board, SQUARE_SIZE)
                if (not self.board.inBounds(self.coords[0], self.coords[1]) or not self.board.isCurrentMove or (self.board.isOnline and not self.board.onlineTurn)):
                    self.currentState = self.gameState.STANDBY
